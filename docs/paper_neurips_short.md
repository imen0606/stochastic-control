...
We build on Bilokon (2026), who synthesises ideas from regime switching (Hamilton, 1989), stochastic control with switching costs, and information theory into a unified framework: regimes defined relative to a strategy's PnL as optimal binary filtration compressions, with existence and uniqueness proofs under Markov structure. The individual ingredients — regime switching models, Bellman dynamic programming with switching costs, and the information bottleneck — are well-established. The contribution of the framework is the specific combination: formalising the practitioner intuition that regimes are strategy-specific (not market-intrinsic) and proving that a single bit, optimally chosen, suffices as a predictive state variable for strategy performance. This formulation is what makes exact Bellman solutions tractable for our gym.
